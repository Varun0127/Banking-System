...
/**
 * Servlet implementation class DebitHistory
 */
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (session == null) {
			...
			try {
				...
				ResultSet rs=st.executeQuery("select * from debit");
				
				request.getRequestDispatcher("debithistory.html").include(request, response);
				out.println("<table style='width: 100%; text-align: center; border: 2px solid rgb(255, 196, 0);'>");
				out.println("<tr style='background-color: rgb(255, 196, 0); color: white; height: 30px;'><th>S.No.</th><th>Name</th><th>Amount</th><th>Date</th></tr>");
				int sno=1;
				while(rs.next()) {	
